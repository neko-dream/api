...
// PolicyConsentParams is parameters of policyConsent operation.
type PolicyConsentParams struct {
	PolicyVersion string
	XFowarderdFor OptString
	UserAgent     OptString
}

func unpackPolicyConsentParams(packed middleware.Parameters) (params PolicyConsentParams) {
	{
		key := middleware.ParameterKey{
			Name: "policyVersion",
			In:   "query",
		}
		params.PolicyVersion = packed[key].(string)
	}
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.XFowarderdFor = v.(OptString)
		}
	}
	{
		key := middleware.ParameterKey{
...
func decodePolicyConsentParams(args [0]string, argsEscaped bool, r *http.Request) (params PolicyConsentParams, _ error) {
	q := uri.NewQueryDecoder(r.URL.Query())
	h := uri.NewHeaderDecoder(r.Header)
	// Decode query: policyVersion.
	if err := func() error {
		cfg := uri.QueryParameterDecodingConfig{
			Name:    "policyVersion",
			Style:   uri.QueryStyleForm,
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotXFowarderdForVal string
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotXFowarderdForVal = c
					return nil
				}(); err != nil {
					return err
				}
				params.XFowarderdFor.SetTo(paramsDotXFowarderdForVal)
				return nil
			}); err != nil {
				return err
			}
