package talksession_command_test
...
type EditCommandTestData struct {
	EditCommand      talksession_command.EditCommand
...
func TestEditCommand_Execute(t *testing.T) {
	// 必要な環境変数を設定
	...
	// コマンドハンドラの初期化
	editCommand := talksession_command.NewEditCommand(
		talkSessionRepo,
		userRepo,
		dbManager,
	)
	...
	testCases := []*txtest.TransactionalTestCase[EditCommandTestData]{
		{
			Name: "正常系: オーナーがトークセッションを編集できる",
			SetupFn: func(ctx context.Context, data *EditCommandTestData) error {
				// オーナーユーザーの作成
			...
			TestFn: func(ctx context.Context, data *EditCommandTestData) error {
				// 編集用の入力データ
				input := talksession_command.EditCommandInput{
				...
				// コマンドの実行
				output, err := data.EditCommand.Execute(ctx, input)
				if err != nil {
					return err
				}
		...
		},
		{
			...
			TestFn: func(ctx context.Context, data *EditCommandTestData) error {
				...
				_, err := data.EditCommand.Execute(ctx, input)
		...
		},
		{
			...
			TestFn: func(ctx context.Context, data *EditCommandTestData) error {
				...
				// EditCommandInputで指定していないフィールドはnilになる
...
// TestEditCommandInput_Validate 入力値バリデーションのユニットテスト
func TestEditCommandInput_Validate(t *testing.T) {
	tests := []struct {
		name    string
		input   talksession_command.EditCommandInput
		wantErr error
